...
describe('Lennard Jones force', () => {
    it("Force is zero at distance equal to r0", () => {
        ...
        const actions = Physics.ForceMap.lennardJones(d, data, params);
        ...
        expect(Physics.Vector.norm(fa)).toBeCloseTo(0, 14);
        expect(Physics.Vector.scale(-1, fa)).toEqual(fb);
    })
    ...
    it("Force is attractive at distance greater than sigma * 2^(1/6)", () => {
        ...
        const fa = actions[0][1];
        const fb = actions[1][1];
        const ab = Physics.Vector.sub(b.ri, a.ri);
        const ba = Physics.Vector.scale(-1, ab);
        // this dot product is positive if the force on
        // a is towards b, negative if the force points
        // away from b
        expect(Physics.Vector.dot(ab, fa)).toBeGreaterThan(0);
        // this dot product checks the same thing, but for
        // node b instead
        expect(Physics.Vector.dot(ba, fb)).toBeGreaterThan(0);
    })
    ...
    it("Force is repelling at distance less than r0", () => {
        ...
        const ab = Physics.Vector.sub(b.ri, a.ri);
        const ba = Physics.Vector.scale(-1, ab);
        expect(Physics.Vector.dot(ab, fa)).toBeLessThan(0);
        expect(Physics.Vector.dot(ba, fb)).toBeLessThan(0);
    })
    ...
    it("Force should be very small at distance much larger than r0", () => {
        ...
        const ab = Physics.Vector.sub(b.ri, a.ri);
        const ba = Physics.Vector.scale(-1, ab);
        expect(Physics.Vector.norm(fa)).toBeLessThan(1e-16);
        expect(Physics.Vector.scale(-1, fa)).toEqual(fb);
    })
    ...
    it("Force should be very large at distance much smaller than r0", () => {
        ...
        const ab = Physics.Vector.sub(b.ri, a.ri);
        const ba = Physics.Vector.scale(-1, ab);
        expect(Physics.Vector.norm(fa)).toBeGreaterThan(1e+20);
        expect(Physics.Vector.scale(-1, fa)).toEqual(fb);
    })
})
