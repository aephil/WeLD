...
var Renderer = function Renderer() {
  ...
  this.setUI = function (u) {
    ...
    ui.canvas.addEventListener("click", function (event) {
      ...
      lattice.data.forEach(function (n) {
        ...
        var isHit = Math.abs((0, _helpers.centreToScreenXPeriodic)(onScreenPos.x, ui.canvas.width) - mouseX) < n.r && Math.abs((0, _helpers.centreToScreenYPeriodic)(onScreenPos.y, ui.canvas.height) - mouseY) < n.r;
  ...
  var cameraView = function cameraView(d) {
    if (ui.highlighted() !== false) {
      var origin = lattice.data[ui.highlighted()];
      var translated = translateVec(d, {
        x: -origin.ri.x,
        y: -origin.ri.y,
        z: -origin.ri.z
      });
      return (0, _helpers.rotY)((0, _helpers.rotX)(translated, theta), rho);
    }

    return (0, _helpers.rotY)((0, _helpers.rotX)(d, theta), rho);
  };

  var update = function update() {
    // node updates
    lattice.data.forEach(function (d) {
      nodeUpdates.forEach(function (fn) {
        fn(d, lattice.data);
      });
    }); // updates
      ...
      if (Array.isArray(forces) && forces.length) {
        ...
        for (var i = 0; i < forces.length; i++) {
          ...
          if (force.name == "valenceAngle") {
            ...
            ui.infoBox.innerHTML += "&emsp;&emsp;Neighbours:</br>&emsp;&emsp;&emsp;" + force.params[2] + " (" + lattice.data[force.params[2]].name + ")" + ", " + force.params[3] + " (" + lattice.data[force.params[3]].name + ")" + "</br>";
  ...
  var drawToolTip = function drawToolTip() {
    ...
    lattice.data.forEach(function (n) {
      ...
      var isHit = Math.abs((0, _helpers.centreToScreenXPeriodic)(imagePos.x, ui.canvas.width) - mouseX) < n.r && Math.abs((0, _helpers.centreToScreenYPeriodic)(imagePos.y, ui.canvas.height) - mouseY) < n.r;
  ...
  var defaultDrawCall = function defaultDrawCall() {
    var ctx = ui.canvas.getContext("2d");
    ctx.clearRect(0, 0, ui.canvas.width, ui.canvas.height);
    ctx.fillStyle = "cornsilk";
    ctx.fillRect(0, 0, ui.canvas.width, ui.canvas.height); // need to copy and reorder ids for sorting draw order

    var forSort = [];
    lattice.data.forEach(function (node) {
    ...
    forSort.forEach(function (id) {
      var n = lattice.data[id];

      if (n.visible) {
        var imagePos = cameraView(n.ri);
        ctx.beginPath();
        ctx.arc((0, _helpers.centreToScreenXPeriodic)(imagePos.x, ui.canvas.width), (0, _helpers.centreToScreenYPeriodic)(imagePos.y, ui.canvas.height), n.r, 0, 2 * Math.PI);
        ctx.closePath();
        ctx.fillStyle = n.col;
        ctx.lineWidth = 1;
        ...
        if (lattice.showEdges()) {
          n.forces.forEach(function (force) {
            if (force.name == "spring") {
              if (n.showEdges) {
                var imagePos1 = cameraView(lattice.data[force.params[2]].ri);
                ctx.beginPath(); // Start a new path

                ctx.moveTo((0, _helpers.centreToScreenXPeriodic)(imagePos.x, ui.canvas.width), (0, _helpers.centreToScreenYPeriodic)(imagePos.y, ui.canvas.height));
                ctx.lineTo((0, _helpers.centreToScreenXPeriodic)(imagePos1.x, ui.canvas.width), (0, _helpers.centreToScreenYPeriodic)(imagePos1.y, ui.canvas.height));
                ctx.closePath();
